...
function srz_yt_enqueue_script() {
	wp_enqueue_script( 'srzmodernizr', srz_yt_get_resource_url( 'js/modernizr.js' ) );
	wp_enqueue_script( 'jquery' );
	wp_enqueue_script( 'srzmp', srz_yt_get_resource_url( 'js/mag-popup.js' ), array( 'jquery' ) );
	wp_enqueue_script( 'srzelastislide', srz_yt_get_resource_url( 'js/jquery.elastislide.min.js' ), array( 'jquery' ) );
	...
	wp_enqueue_style( 'srzelastislidercss', srz_yt_get_resource_url( 'css/elastislide.min.css' ) );
...
function srz_yt_album_shortcode( $atts ) {
	...
	if ( is_array( $videos ) ) {
		...
		if ( $album['liststyle'] == 'fullpage' ) {
			...
			include 'srizon-yt-album-respslider.php';
		}
	}
...
function srz_yt_find_type($id, $key){
	...
	$url = 'https://www.googleapis.com/youtube/v3/playlists?id='.$id.'&key='.$key.'&part=id';
	srz_yt_debug_msg('Trying to get data from:','<a href="'.$url.'" target="_blank">'.$url.'</a>');
	$json_str = wp_remote_get($url,array( 'timeout' => 30));
	$json = json_decode($json_str['body']);
...
function srz_yt_get_videos_from_pl_id($pl_id,$key,&$videos,$total){
	$i = 0;
	$page_token= '';
	while(true) {
		$url = 'https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&key=' . $key . '&playlistId=' . $pl_id . '&fields=nextPageToken,pageInfo,items(snippet(title,description,resourceId/videoId,thumbnails/medium/url))&maxResults=50'.$page_token;
		srz_yt_debug_msg( 'Trying to get data from:', '<a href="' . $url . '" target="_blank">' . $url . '</a>' );
		$json_str = wp_remote_get( $url, array( 'timeout' => 30 ) );
		$json     = json_decode( $json_str['body'] );
...
function srz_yt_get_album_api( $album ) {
	...
	if ( ! count($videos) or isset( $_GET['forcesyncyt'] ) ) {
		...
		if($pl_id !== false){
			$videos = srz_yt_get_videos_from_pl_id($pl_id,$album['api_key'],$videos,$album['totalvid']);
		}
		if ( ! count($videos) ) {
